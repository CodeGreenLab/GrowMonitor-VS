...
//Criar funções para ligar/desligar a bomba
void ligarBomba() {
  Serial.println("🔌 Tentando ligar a bomba...");
  digitalWrite(RELE_BOMBA, HIGH); // Liga o relé (ativo em LOW)
  Serial.println("💧 Bomba de água LIGADA! (GPIO26: " + String(digitalRead(RELE_BOMBA)) + ")");
  enviarMensagemTelegram("💧 A bomba de água foi LIGADA!");
}

void desligarBomba() {
  Serial.println("🔌 Tentando desligar a bomba...");
  digitalWrite(RELE_BOMBA, LOW); // Desliga o relé
  Serial.println("💧 Bomba de água DESLIGADA! (GPIO26: " + String(digitalRead(RELE_BOMBA)) + ")");
  enviarMensagemTelegram("💧 A bomba de água foi DESLIGADA!");
}
...
void configurarComandosTelegram() {
  ...
  String jsonBody = F("{\"commands\":["
                        ...
                        "{\"command\":\"alertatemperatura\",\"description\":\"Escreva alertatemperatura xx para mudar o limite da temperatura interna\"},"
                        "{\"command\":\"bombaligar\",\"description\":\"Liga a bomba de água\"},"
                        "{\"command\":\"bombadesligar\",\"description\":\"Desliga a bomba de água\"}"
                       "]}");
